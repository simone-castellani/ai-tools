...
def validate_skill(skill_path):
    ...
    if allowed_tools:
        parsed_tools = _parse_allowed_tools(allowed_tools)
        if parsed_tools is None:
            return False, "allowed-tools must be a string or a list of strings"
        unknown_tools = sorted(
            {tool for tool in parsed_tools if tool not in OPEN_CODE_TOOL_ALIASES}
        )
        if unknown_tools:
            accepted_tools = ", ".join(sorted(OPEN_CODE_TOOL_ALIASES))
            return False, (
                f"Unknown tool name(s) in allowed-tools: {', '.join(unknown_tools)}. "
                f"Accepted OpenCode names and aliases: {accepted_tools}"
            )
